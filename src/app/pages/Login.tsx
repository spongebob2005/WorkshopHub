...
import { Eye, EyeOff, Mail, Lock, Sparkles, BookOpen, Users, Award, Shield } from 'lucide-react';
...
export const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (success) {
      toast.success(`Welcome back! ${loginType === 'admin' ? 'Admin' : 'Student'} login successful.`);
      navigate(loginType === 'admin' ? '/admin' : '/', { replace: true });
    } else {
          ...
          <div className="bg-white rounded-3xl shadow-xl shadow-gray-200/60 border border-gray-100 p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <Button
                type="submit"
                disabled={isLoading}
                className={`w-full h-11 rounded-xl border-0 shadow-lg text-base transition-all ${
                  loginType === 'admin'
                    ? 'bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 shadow-purple-200/50 hover:shadow-purple-300/50'
                    : 'bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 shadow-indigo-200/50 hover:shadow-indigo-300/50'
                }`}
              >
                {isLoading ? (
            ...
            {loginType === 'admin' && (
              <p className="text-center text-xs text-gray-400 mt-6">
                Demo: admin@workshophub.com / admin123
              </p>
            )}
          </div>
        </div>
